...
- **局域网平板手写**：在平板浏览器打开设置中的手写板链接，用手写笔书写、单指连续滑动、双指缩放；画笔与手指书写偏好跨纸张保留。
...
Codex 对话和翻译复用后台进程。翻译使用精简的文本上下文，未指定模型时优先选择可用的快速模型。内置 PDF 工具复用已解析的文档和页文本；CLI 兼容模式通过临时本地 MCP 连接使用同样的工具，无需依赖 `pdftotext`。消息支持文件引用、本地图片、代码块和 LaTeX 显示。

## 数据与隐私

Tessel 将工作区保存在 Electron 的 `userData` 目录中，其中包含 PDF 元数据、阅读状态、对话、翻译、笔记、固定内容和偏好设置。PDF 文件本身始终保留在原始位置。WebDAV 同步只有在用户配置后才会启用。

当前版本详情请查看[发布日志](docs/releases/v1.5.2.md)。
